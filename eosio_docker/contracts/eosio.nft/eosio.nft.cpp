                ...
                string memo)
{
  ...
  {
    mint(to, st.issuer, asset{1, symbol}, uri, name, metadata);
  }
...
void nft::approve(account_name authorizer,
                  ...
                  id_type id)
{
  ...
  // Ensure owner owns token
  eosio_assert(sender_token->owner == authorizer, "sender does not own token with specified ID");
...
void nft::transferfrom( account_name spender,
                        account_name from,
                        account_name to,
                        id_type id,
                        string memo)
{
  ...
  // Ensure spender can spend token
  eosio_assert(sender_token->spender == spender, "spender not authorized to spend token with specified ID");
  ...
  // Transfer NFT from sender to receiver
  tokens.modify(st, spender, [&](auto &token) {
    token.owner = to;
    token.spender = 0;
  });
...
EOSIO_ABI(nft, (create)(issue)(transfer)(approve)(transferfrom)(setrampayer)(burn))
